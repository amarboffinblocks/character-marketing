...
export async function GET(
  _: Request,
  context: { params: Promise<{ requestId: string; assetType: string; assetId: string }> }
) {
  ...
  const { requestId, assetType, assetId } = await context.params

  try {
    const item = await fetchOrderDeliverableItemForViewer({
      orderId: asString(requestId),
      viewerId: user.id,
