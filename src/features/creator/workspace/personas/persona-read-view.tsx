...
export function PersonaReadView({
  entityId,
  apiPathPrefix = "/api/creator/personas",
  backHref = "/dashboard/creator/workspace/personas",
  backLabel = "Back to Personas",
}: PersonaReadViewProps) {
  const searchParams = useSearchParams()
  const id = entityId ?? searchParams.get("id")?.trim() ?? ""
  const [item, setItem] = useState<CreatorPersona | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const payload = (await response.json()) as { item?: CreatorPersona | null }
        if (!mounted) return
  ...
  if (isLoading) return <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">Loading persona...</CardContent></Card>
  if (!item) return <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">Persona not found.</CardContent></Card>
            ...
            <div className="relative z-10 -mt-12 shrink-0 self-start">
              <div className="flex size-24 items-center justify-center overflow-hidden rounded-full border-4 border-card bg-muted shadow-md ring-2 ring-background sm:size-28">
                {item.avatarUrl ? (
                  ...
                  <img src={item.avatarUrl} alt={item.personaName} className="h-full w-full object-cover" />
                ) : (
                  <span className="text-xl font-semibold text-primary">{(item.personaName.trim().slice(0, 2) || "PR").toUpperCase()}</span>
                )}
              </div>
            </div>
            <div className="min-w-0 space-y-1.5">
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="max-w-[170px] truncate text-xl font-semibold tracking-tight sm:max-w-none sm:text-3xl">{item.personaName}</h2>
              ...
              <p className="text-sm text-muted-foreground whitespace-pre-wrap">{item.personaDetails}</p>
            ...
            <div className="overflow-hidden rounded-xl border border-border/70 bg-card p-3">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold">{item.personaName}</p>
                  <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{item.personaDetails}</p>
                </div>
              </div>
        ...
        <div className="space-y-4">
          ...
          {/* <Card>
            <CardHeader className="border-b pb-4">
              ...
              <CardDescription>Current persona usage count</CardDescription>
            </CardHeader>
            <CardContent className="py-4 text-sm text-muted-foreground">{item.usageCount}</CardContent>
          </Card> */}
        </div>
      </section>
    </div>
